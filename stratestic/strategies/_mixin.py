...
class StrategyMixin:
    ...
    def set_model(self, model, feature_columns=None):
        ...
        This allows ML-based strategies to use external models trained with
        any framework (scikit-learn, PyTorch, TensorFlow, etc.).
        ...
        feature_columns : list of str, optional
            Names of the feature columns the model expects. If None, all
            columns in the data will be used.
        ...
        self._feature_columns = feature_columns if feature_columns is not None else []
    
    def predict(self, features):
        """
        Use the attached ML model to make predictions.
        
        Parameters
        ----------
        features : pd.DataFrame or np.ndarray
            Feature data to predict on. Should match the format the model
            was trained on.
        ...
        ------
        ValueError
            If no model has been attached via set_model().
            
        Examples
        --------
        >>> strategy.set_model(trained_model, feature_columns=['ma_fast', 'ma_slow'])
        >>> predictions = strategy.predict(data[['ma_fast', 'ma_slow']])
        """
        if self._ml_model is None:
            raise ValueError(
                "No ML model attached. Use set_model() to attach a trained model first."
            )
