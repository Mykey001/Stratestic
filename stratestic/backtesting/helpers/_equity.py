...
def calculate_static_equity_panel(
    ...
    contributions = np.zeros((n_bars, n_symbols))

    equity = amount
    notional = np.zeros(n_symbols)
    entry_notional = np.zeros(n_symbols)
    prev_side = np.zeros(n_symbols)
    contribution = np.zeros(n_symbols)  # cumulative per-symbol pnl (marks - costs)
    wiped = False
    # on portfolio wipeout each symbol's equity (share + contribution) is 0,
    # so the per-symbol curves keep summing to the floored portfolio equity
    wipe_contribution = -amount / n_symbols

    for i in range(n_bars):
        if wiped:
            for j in range(n_symbols):
                contributions[i, j] = contribution[j]
            continue
        ...
        for j in range(n_symbols):
            ...
            delta = prev_side[j] * (growth - 1.0) * notional[j]
            equity = equity + delta
            contribution[j] = contribution[j] + delta
        ...
        if equity <= 0.0:
            ...
            equity = 0.0
            for j in range(n_symbols):
                contribution[j] = wipe_contribution
                contributions[i, j] = wipe_contribution
            continue

        # ---- Phase 3: entries, sized off the same post-exit equity but
        # bounded by the leverage headroom. Each entry is allocated capital
        # `w * equity * leverage`; held positions are never rebalanced, so a
        # new entry can only use the capital their (compounded) notionals
        # leave free. The notional embeds the entry cost, so it degenerates
        # to the single-symbol static model exactly (one position fills its
        # whole budget, no scaling). ----
        equity_mid = equity
        budget = equity_mid * leverage

        held_capital = 0.0
        for j in range(n_symbols):
            held_capital += notional[j]  # exited positions are 0 after Phase 2

        available = budget - held_capital
        if available < 0.0:
            available = 0.0
        ...
        for j in range(n_symbols):
            ...
            if new_side != 0.0 and new_side != prev_side[j] and not liquidations[i, j]:
                target_capital += weights[i, j] * budget

        scale = 1.0
        if target_capital > available and target_capital > 0.0:
            scale = available / target_capital

        for j in range(n_symbols):
            new_side = sides[i, j]
            if new_side != 0.0 and new_side != prev_side[j] and not liquidations[i, j]:
                capital = scale * weights[i, j] * budget
                notional[j] = capital / (1.0 + trading_costs * new_side)
    ...
    return equity_arr, notionals_out, contributions
