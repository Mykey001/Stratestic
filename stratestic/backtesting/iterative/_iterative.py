...
class IterativeBacktester(BacktestMixin, Trader):
    ...
    def _test_strategy(
        ...
        data = self._trim_panel_data(self._get_data()) if self.is_panel else self._get_data().dropna().copy()
    ...
    def _iterative_backtest_panel(self, data):
        ...
        contributions_mat = np.zeros((n_bars, n_symbols))
        listed_mat = np.zeros((n_bars, n_symbols), dtype=bool)

        equity = float(self.amount)
        notional = np.zeros(n_symbols)
        prev_side = np.zeros(n_symbols)
        contribution = np.zeros(n_symbols)
        wipe_contribution = -float(self.amount) / n_symbols
        wiped = False
        ...
        for i, (timestamp, row) in enumerate(data.iterrows()):
            ...
            for j, symbol in enumerate(symbols):
                ...
                return_nan = pd.isna(bar_return)
                returns_mat[i, j] = 0.0 if (i == 0 or return_nan) else bar_return
                listed_mat[i, j] = (not return_nan) or (i > 0 and listed_mat[i - 1, j])

                # pre-listing / no-return bars are flat, mirroring
                # _prepare_panel_arrays' masking in the vectorized engine
                sides_mat[i, j] = 0.0 if return_nan else signals[symbol]

            weights_mat[i, :] = self._get_row_weights(row, symbols, sides_mat[i, :])

            if wiped:
                contributions_mat[i, :] = contribution
            ...
            for j in range(n_symbols):
                if sides_mat[i, j] != prev_side[j] and prev_side[j] != 0.0:
                    ...
                    contribution[j] = contribution[j] - self.tc * notional[j]
                    notional[j] = 0.0
                    prev_side[j] = 0.0
            ...
            if equity <= 0.0:
                wiped = True
                equity = 0.0
                contribution[:] = wipe_contribution
                contributions_mat[i, :] = wipe_contribution
                continue

            # ---- Phase 3: entries, bounded by the leverage headroom (mirrors
            # calculate_static_equity_panel exactly) ----
            equity_mid = equity
            budget = equity_mid * leverage

            held_capital = notional.sum()  # exited positions are 0 after Phase 2
            available = max(0.0, budget - held_capital)

            target_capital = 0.0
            for j in range(n_symbols):
                new_side = sides_mat[i, j]
                if new_side != 0.0 and new_side != prev_side[j]:
                    target_capital += weights_mat[i, j] * budget
            ...
            for j in range(n_symbols):
                new_side = sides_mat[i, j]
                if new_side != 0.0 and new_side != prev_side[j]:
                    capital = scale * weights_mat[i, j] * budget
                    notional[j] = capital / (1.0 + self.tc * new_side)
                    equity = equity - self.tc * notional[j]
                    contribution[j] = contribution[j] - self.tc * notional[j]
