...
class TestPanelLiquidation:
    ...
    @pytest.mark.parametrize("backtester_class", [VectorizedBacktester, IterativeBacktester])
    def test_breach_on_flip_bar_does_not_suppress_new_position(self, backtester_class, mocked_plotly_figure_show):
        """Regression (#5): a margin breach exactly on a trade's flip bar must
        not flag the next position - the long exits via the flip and the short
        still opens."""
        # at 10x a drop to 90 breaches the BTC long's margin, and that same bar
        # is where it flips to short
        panel = build_panel({
            "BTCUSDT": make_flat_ohlc([100, 100, 100, 90, 90, 90]),
        ...
        btc_trades = [t for t in backtester.trades if t.symbol == "BTCUSDT"]
        assert any(t.side == -1 for t in btc_trades), "the short after the flip must exist"

    def test_engines_agree_on_flip_bar_breach(self, mocked_plotly_figure_show):
        panel = build_panel({
            "BTCUSDT": make_flat_ohlc([100, 100, 100, 90, 90, 90]),
            "ETHUSDT": make_flat_ohlc([100, 100, 100, 100, 100, 100]),
        })
