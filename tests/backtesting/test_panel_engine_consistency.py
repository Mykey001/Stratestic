...
class TestPanelEngineConsistency:
    ...
    def test_engines_agree(self, trading_costs, weights, mocked_plotly_figure_show):
        ...
        # per-symbol equity contributions are equal across engines and sum to
        # the portfolio equity
        equity_cols = [f"equity_{symbol}" for symbol in PATTERNS]
        for col in equity_cols:
            pd.testing.assert_series_equal(vect.processed_data[col], ite.processed_data[col])

        per_symbol_sum = vect.processed_data[equity_cols].sum(axis=1)
        pd.testing.assert_series_equal(
            per_symbol_sum, vect.processed_data["equity"], check_names=False
        )
