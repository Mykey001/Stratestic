...
class TestBroadcastStrategy:
    ...
    def test_sides_match_independent_runs(self):
        ...
        for symbol, frame in frames.items():
            ...
            # the broadcast emits NaN on each symbol's warm-up bars (where its
            # indicators aren't ready); on the valid bars it matches an
            # independent single-symbol run
            valid = data[(symbol, SIDE)].dropna()
            pd.testing.assert_series_equal(
                valid.astype(int), solo_sides.loc[valid.index], check_names=False
            )
