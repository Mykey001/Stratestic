...
class TestPanelHandComputed:
    ...
    @pytest.mark.parametrize("backtester_class", [VectorizedBacktester, IterativeBacktester])
    def test_late_listing_keeps_early_symbol_history(self, backtester_class, mocked_plotly_figure_show):
        """Regression (#2): a symbol with a shorter history must not trim the
        whole panel to the common window - the longer-history symbol trades
        from the start, the late one only once listed."""
        btc = make_flat_ohlc([100, 100, 110, 120, 130, 140])
        eth = make_flat_ohlc([np.nan, np.nan, np.nan, 100, 100, 110])  # lists at bar 3

        panel = build_panel({"BTCUSDT": btc, "ETHUSDT": eth}, join="outer")
        patterns = {"BTCUSDT": [0, 1, 1, 1, 1, 1], "ETHUSDT": [0, 0, 0, 1, 1, 1]}
        weights = {"BTCUSDT": 0.5, "ETHUSDT": 0.5}  # leave headroom for ETH

        strategy = PresetSidesPanel(patterns, weights=weights, data=panel)
        backtester = backtester_class(strategy)
        backtester.run(print_results=False, plot_results=False)

        # the panel was NOT trimmed to ETH's listing: BTC trades across the
        # full window (5 processed bars: bar 0 is dropped for the NaN return)
        assert len(backtester.processed_data) == 5
        symbols_traded = {t.symbol for t in backtester.trades}
        assert symbols_traded == {"BTCUSDT", "ETHUSDT"}

        # ETH is flat (side 0) before it lists
        assert (backtester.processed_data["side_ETHUSDT"].iloc[:2] == 0).all()

    def test_late_listing_engines_agree(self, mocked_plotly_figure_show):
        btc = make_flat_ohlc([100, 100, 110, 120, 130, 140])
        eth = make_flat_ohlc([np.nan, np.nan, np.nan, 100, 100, 110])
        panel = build_panel({"BTCUSDT": btc, "ETHUSDT": eth}, join="outer")
        patterns = {"BTCUSDT": [0, 1, 1, 1, 1, 1], "ETHUSDT": [0, 0, 0, 1, 1, 1]}
        ...
        ite = IterativeBacktester(strategy)
